...
export const getSetupAddresses = async (
  ...
  if (network.name === "hardhat") {
    if (!deployments)
      throw Error(
        `Must specify contract deployments for ${network.name} network`
      );
    return {
      gnosisSingleton: (await deployments.get("GnosisSafe"))?.address,
      gnosisFallbackLibrary: (
        await deployments.get("CompatibilityFallbackHandler")
      )?.address,
      gnosisMultisendLibrary: (await deployments.get("MultiSend"))?.address,
      poster: (await deployments?.get("Poster")).address,
      gnosisSafeProxyFactory: (await deployments.get("GnosisSafeProxyFactory"))
        ?.address,
      moduleProxyFactory: (await deployments.get("ModuleProxyFactory"))
        ?.address,
  ...
  const gnosisSingleton =
    getSafeSingletonDeployment(filter)?.networkAddresses[chainId];
  const gnosisFallbackLibrary =
    getCompatibilityFallbackHandlerDeployment(filter)?.networkAddresses[
      chainId
    ];
  const gnosisMultisendLibrary =
    getMultiSendDeployment(filter)?.networkAddresses[chainId];
  const gnosisSafeProxyFactory =
    getProxyFactoryDeployment(filter)?.networkAddresses[chainId];
  let moduleProxyFactory = Object.values(SupportedNetworks).includes(
    Number(chainId)
  )
